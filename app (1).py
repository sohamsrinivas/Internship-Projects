...
with st.sidebar:
    ...
    threshold = st.slider(
        ...
        step=0.01,
        help="Adjust the classification cutoff."
    )
    st.markdown("---")
    st.markdown("**Model**")
...
for col, (label, val, fmt) in zip(cols, kpis):
    with col:
        st.markdown(kpi(label, val, fmt), unsafe_allow_html=True)
...
st.dataframe(
    filtered_rules[display_cols].rename(columns=display_labels)
...
# Fix: use map instead of deprecated applymap, style Correct column as text
...
display_df = show_df[["Customer_ID","Actual_Label","Dynamic_Pred","Predicted_Probability",
                       "Segment","Campaign_Response","Recommendation_Category","Correct"]].rename(
    columns={"Dynamic_Pred":"Predicted","Predicted_Probability":"Probability"}
)

st.dataframe(
    display_df.style
        .format({"Probability":"{:.3f}"})
        .map(style_correct, subset=["Correct"]),
    use_container_width=True,
    height=350
)
...
with col1:
    ...
    csv_pred = show_df.to_csv(index=False).encode("utf-8")
    st.download_button(
...
with col2:
    st.markdown("**Download Filtered Rules**")
    csv_rules = filtered_rules[display_cols].rename(columns=display_labels).to_csv(index=False).encode("utf-8")
    st.download_button(
